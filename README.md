...
After connection retries are exhausted, phone speech continues for the rest of the passage. Before speaking the next passage, Reciter automatically tries the selected Edge voice again by requesting its first unread segment. A failed speech request or audio playback also falls back for the remainder of that passage, then retries at the next passage. An ongoing connection check is allowed to finish rather than being restarted at passage boundaries. Connect can still retry manually. iPhone may request another tap when moving from phone speech to Edge: the existing playback-permission message asks for Play to retry the unread segment. This handoff still needs device verification.
