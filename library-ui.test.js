...
test('package import preview, topic tap, shared pause and automatic priority playback', async t => {
  ...
  let blob, exportCount = 0;
  p.w.URL.createObjectURL = value => { blob = value; return 'blob:progress-' + ++exportCount; }; p.w.URL.revokeObjectURL = () => {};
  p.$('export-progress').click(); await until(() => !p.$('download-progress').hidden);
  assert.equal(p.$('download-progress').textContent, 'Save progress file');
  assert.match(p.$('download-progress').download, /^reciter-progress-.*\.json$/);
  ...
  await until(() => !p.$('export-progress').disabled);
  p.$('export-progress').click(); await until(() => p.$('download-progress').href === 'blob:progress-2');
  assert.equal(p.$('download-progress').hidden, false);
  assert.equal(p.$('download-progress').textContent, 'Save progress file');
